...
class TestCreditBalanceAdjustment(unittest.TestCase):
    ...
    def test_endpoint(self):
        creditBalanceAdjustment = invoiced.CreditBalanceAdjustment(self.client, 123)
        self.assertEqual('/credit_balance_adjustments/123', creditBalanceAdjustment.endpoint())

    @responses.activate
    def test_create(self):
        responses.add('POST', 'https://api.invoiced.com/credit_balance_adjustments',
                      status=201,
        ...
        self.assertIsInstance(creditBalanceAdjustment, invoiced.CreditBalanceAdjustment)
        self.assertEqual(creditBalanceAdjustment.id, 123)
        self.assertEqual(creditBalanceAdjustment.name, "Alpha")

    @responses.activate
    def test_retrieve(self):
        responses.add('GET', 'https://api.invoiced.com/credit_balance_adjustments/123',
                      status=200,
                      json={"id": "123", "name": "Alpha"})
    ...
    def test_update(self):
        responses.add('PATCH', 'https://api.invoiced.com/credit_balance_adjustments/123',
                      status=200,
    ...
    @responses.activate
    def test_list(self):
        responses.add('GET', 'https://api.invoiced.com/credit_balance_adjustments',
                      status=200,
                      json=[{"id": 123, "name": "Alpha"}],
                      adding_headers={
                        'x-total-count': '10',
    ...
    def test_delete(self):
        responses.add('DELETE', 'https://api.invoiced.com/credit_balance_adjustments/123',
                      status=204)
